...
// Get user timezone from store or default
function getUserTimezone(): string {
  try {
    const stored = localStorage.getItem('olive-baby-timezone');
    if (stored) {
      const parsed = JSON.parse(stored);
      return parsed.state?.timezone || DEFAULT_TIMEZONE;
    }
  } catch {
    // Ignore errors
  }
...
// Format date to Brazilian format (timezone-aware)
export function formatDateBR(date: string | Date, timezone?: string): string {
  const tz = timezone || getUserTimezone();
  return formatDateInTimezone(date, tz);
}

// Format time to Brazilian format (timezone-aware)
export function formatTimeBR(date: string | Date, timezone?: string): string {
  const tz = timezone || getUserTimezone();
  return formatTimeInTimezone(date, tz);
}

// Alias para formatTime (compatibilidade)
export function formatTime(date: string | Date, timezone?: string): string {
  return formatTimeBR(date, timezone);
}

// Format date and time (timezone-aware)
export function formatDateTimeBR(date: string | Date, timezone?: string): string {
  const tz = timezone || getUserTimezone();
