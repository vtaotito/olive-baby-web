...
export function RoutineHistoryPage() {
  ...
  const handleDelete = () => {
    if (selectedRoutine) deleteMutation.mutate(selectedRoutine.id);
  };
      ...
      <DashboardLayout>
        <div className="p-6">
          <div className="text-center py-12">
            ...
            <Button onClick={() => window.location.href = '/settings/babies'}>Cadastrar Bebê</Button>
          </div>
        </div>
      </DashboardLayout>
    );
      ...
      <div className="p-6 max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button onClick={() => window.location.href = '/routines/feeding'} className="flex items-center gap-2">
          ...
          <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mt-6">
            ...
            {Object.entries(ROUTINE_TYPES).map(([type, { label, icon }]) => (
              <div key={type} className="bg-white rounded-lg p-4 shadow-sm border">
                <div className="text-sm text-gray-500 flex items-center gap-1"><span>{icon}</span> {label}</div>
                <div className="text-2xl font-bold text-gray-900">{stats.byType[type] || 0}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border p-4 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div>
              ...
              <Select value={selectedBabyId?.toString() || ''} onChange={(e) => setSelectedBabyId(e.target.value ? parseInt(e.target.value) : null)}>
            ...
            <div>
              ...
              <Select value={routineType} onChange={(e) => { setRoutineType(e.target.value as RoutineType | 'ALL'); setPage(1); }}>
                <option value="ALL">Todos</option>
                {Object.entries(ROUTINE_TYPES).map(([type, { label }]) => (<option key={type} value={type}>{label}</option>))}
              </Select>
            </div>
            <div>
            ...
            <div className={cn(dateFilter === 'custom' ? 'col-span-2' : '')}>
              ...
              <div className="relative">
                ...
                <Input type="text" placeholder="Buscar por notas..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10" />
              </div>
            </div>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12"><p className="text-red-600">Erro ao carregar rotinas</p></div>
        ) : filteredRoutines.length === 0 ? (
          <div className="text-center py-12 bg-white rounded-lg shadow-sm border">
            <p className="text-gray-500 mb-4">Nenhuma rotina encontrada</p>
            <Button onClick={() => window.location.href = '/routines/feeding'}>Registrar Primeira Rotina</Button>
          </div>
        ) : (
          <>
                ...
                <div key={date} className="bg-white rounded-lg shadow-sm border">
                  ...
                  <div className="divide-y">
                    {dateRoutines.map((routine: RoutineLog) => {
                      ...
                      const endTime = routine.endTime ? format(parseISO(routine.endTime), 'HH:mm') : null;
                      const duration = routine.durationSeconds ? Math.floor(routine.durationSeconds / 60) : null;
                      ...
                      return (
                        <div key={routine.id}>
                          <div className={cn(
                            ...
                            isEditing ? 'bg-olive-50 ring-1 ring-inset ring-olive-200' : 'hover:bg-gray-50'
                          )}>
                            <div className="flex items-start justify-between">
                              <div className="flex items-start gap-4 flex-1">
                                <div className="flex-shrink-0">
                                  <div className={cn('w-12 h-12 rounded-full flex items-center justify-center text-2xl', routineInfo.color)}>
                                    {routineInfo.icon}
                                  </div>
                                </div>
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-2 mb-1">
                                    <h4 className="font-semibold text-gray-900">{routineInfo.label}</h4>
                                    <Badge className={routineInfo.color}>{routineInfo.label}</Badge>
                                    {routine.updatedAt && routine.updatedAt !== routine.createdAt && (
                                      <Badge variant="secondary" className="text-[10px] px-1.5 py-0 text-gray-400">editado</Badge>
                                    )}
                                  </div>
                                  ...
                                  {routine.notes && <p className="text-sm text-gray-600 mt-2">{routine.notes}</p>}
                              ...
                              <div className="flex items-center gap-2">
                                <Button
                                  ...
                                  title="Editar"
                                  className={isEditing ? 'bg-olive-200 text-olive-700' : ''}
                                >
                                ...
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => { setSelectedRoutine(routine); setShowDeleteModal(true); }}
                                  title="Deletar"
                          ...
                          {isEditing && (
                            <div ref={editFormRef} className="px-6 pb-4 bg-olive-50/50 border-t border-olive-100">
                              <RoutineRecordEditModal
                                isOpen={true}
                                onClose={() => setEditingId(null)}
                                routine={routine}
                                routineType={routine.routineType}
                                onSave={(data) => updateMutation.mutate({ routineId: routine.id, data })}
        ...
        {showDeleteModal && selectedRoutine && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
              ...
              <p className="text-gray-600 mb-6">Tem certeza que deseja deletar esta rotina? Esta ação não pode ser desfeita.</p>
              <div className="flex gap-3 justify-end">
                <Button variant="outline" onClick={() => { setShowDeleteModal(false); setSelectedRoutine(null); }}>Cancelar</Button>
